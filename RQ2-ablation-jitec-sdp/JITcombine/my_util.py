...
from sklearn.metrics import confusion_matrix, roc_auc_score, matthews_corrcoef, precision_recall_fscore_support, \
    classification_report, auc,recall_score,precision_score,f1_score,roc_auc_score, accuracy_score, matthews_corrcoef, precision_recall_curve
import math
import numpy as np
...
def convert_examples_to_features(item, cls_token='[CLS]', sep_token='[SEP]', sequence_a_segment_id=0,
    ...
    if no_abstraction:
        ...
        added_codes = [preprocess_code_line(line, False) for line in file_codes]
    ...
    # m = nn.AvgPool1d(kernel_size=3,stride=1)
    ...
    # input_ids = m(torch.tensor(input_ids).unsqueeze(0).float())
...
class TextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path=None, mode='train'):
        ...
        for commit_id, label, msg, files in zip(commit_ids, labels, msgs, codes):
            if(features_data['commit_hash'].isin([commit_id]).any()):
...
def eval_metrics(result_df):
    ...
    return accuracy,precision,recall,auc_pr,mcc,f1, AUC, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt
...
def eval_result(result_path, features_path):
    ...
    accuracy,precision,recall,auc_pr,mcc,f1, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt = eval_metrics(
        RF_result)
    logging.info(
        'accuracy: {:.4f},precision: {:.4f},recall: {:.4f},auc_pr: {:.4f},mcc:{:.4f},F1: {:.4f}, AUC: {:.4f}, PCI@20%LOC: {:.4f}, Effort@20%Recall: {:.4f}, POpt: {:.4f}'.format(
            accuracy,precision,recall,auc_pr,mcc,f1, auc, recall_20_percent_effort, effort_at_20_percent_LOC_recall, p_opt))
