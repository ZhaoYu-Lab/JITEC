...
data_path = "lime4line_score/result"
...
def pred(data):
    ...
    output_dir = "model/jitfine/saved_models_lime4/checkpoints/checkpoint-best-f1/model.bin"
    checkpoint = torch.load(output_dir)
    args = " "
    model.resize_token_embeddings(len(tokenizer))
...
def main(args):
    ...
    prediction_result = pd.read_csv('model/jitfine/saved_models_lime4/checkpoints/predictions.csv')
    ids = []
    predd = []
    actual = []
    prob = []
    ...
    for commit in correctly_predicted_commit:
        ...
        print(m)
        print(index)
        exp = explainer.explain_instance(m, predict_fn=pred, distance_metric='dice',
