...
# from concat.model import Model
...
data_path = "rcs4line_score/result"
...
def get_LIME_explainer(proj_name, train_feature, feature_names):
    ...
    if not os.path.exists(LIME_explainer_path):
        ...
        explainer = LimeTabularExplainer(train_feature, mode="classification",
...
def eval_line_level(proj_name, best_k_neighbor,model_path):
    # load model here
...
def eval_line_level_at_commit(cur_proj, data_path,line_score_df_col_name,score_cols):
    ...
    for commit in all_commits:
        ...
        for n, agg_method in enumerate(score_cols):
            ...
            IFA, top_20_percent_LOC_recall, effort_at_20_percent_LOC_recall, top_10_acc, top_5_acc = get_line_level_metrics(RF_line_scr, line_label)
    ...
    recall_20_percent_effort_df.to_csv(data_path + 'text_metric_line_eval_result/'+cur_proj+'_recall_20_percent_effort_min_df_3_300_trees.csv') 
    effort_20_percent_recall_df.to_csv(data_path + 'text_metric_line_eval_result/'+cur_proj+'_effort_20_percent_recall_min_df_3_300_trees.csv')
    top_10_acc_df.to_csv(data_path + 'text_metric_line_eval_result/'+cur_proj+'_top_10_acc_min_df_3_300_trees.csv')
    top_5_acc_df.to_csv(data_path + 'text_metric_line_eval_result/' + cur_proj + '_top_5_acc_min_df_3_300_trees.csv')
...
def pred(data):
    ...
    output_dir = "model/jitfine/saved_models_concatrcs4/checkpoints/checkpoint-best-f1/model.bin"
    checkpoint = torch.load(output_dir)
    args = " "
    model.resize_token_embeddings(len(tokenizer))
...
def main(args):
    ...
    explainer = LimeTabularExplainer(X, mode='classification',
                                feature_names=feature_names, 
                                discretize_continuous=False, random_state=3407)
    ...
    prediction_result = pd.read_csv('model/jitfine/saved_models_concatrcs4/checkpoints/predictions.csv')
    ids = []
    predd = []
    actual = []
    prob = []
    for i in prediction_result.index:
        a = prediction_result.iloc[i].loc["0\t1\t2\t3"].split()
        ids.append(a[0])
        predd.append('1' if a[2]=='True' else '0')
        actual.append(a[3])
        prob.append(float(a[1]))
    pp = pd.DataFrame(columns=["test_commit","pred","actual","prob"])
    pp["test_commit"]=ids
    pp["pred"]=predd
    pp["actual"]=actual
    pp["prob"]=prob
    prediction_result=pp
    ...
    print(correctly_predicted_commit)
    ...
    for commit in correctly_predicted_commit:
        ...
        print(m)
        print(index)
        exp = explainer.explain_instance(m, predict_fn=pred, distance_metric="cityblock",
    ...
    #         line_score_list = np.zeros(0) token_list
            ...
            line_stuff.append(line_level_df[line_level_df['commit_id']==commit]['label'])
    ...
    #                 add_agg_scr_to_list(line_stuff, top_k_scr_list)
                add_agg_scr_to_list(line_stuff, list(top_k_scr_list[:len(token_list)]))
            add_agg_scr_to_list(line_stuff,line_score_list)
    ...
    line_score_df_col_name = ['commit_id', 'total_tokens', 'line_level_label'] + score_cols
    line_score_df_col_name = ['commit_id', 'total_tokens', 'line_level_label'] + score_cols
    eval_line_level_at_commit('changes', data_path,line_score_df_col_name,score_cols)
