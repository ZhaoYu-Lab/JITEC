...
def pred(data):
    ...
    output_dir = "model/jitfine/saved_models_concatrcs1/checkpoints/checkpoint-best-f1/model.bin"
    checkpoint = torch.load(output_dir)
    args = " "
    model.resize_token_embeddings(len(tokenizer))
...
def main(args):
    ...
    prediction_result = pd.read_csv('model/jitfine/saved_models_concatrcs1/checkpoints/predictions.csv')
    ids = []
    predd = []
    actual = []
    prob = []
