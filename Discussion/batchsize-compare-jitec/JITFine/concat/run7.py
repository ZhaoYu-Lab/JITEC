...
def pred(data):
    ...
    output_dir = "model/jitfine/saved_models_concatrcs2/checkpoints/checkpoint-best-f1/model.bin"
    checkpoint = torch.load(output_dir)
    args = " "
    model.resize_token_embeddings(len(tokenizer))
...
def deal_with_attns(item, attns, predd, commit2codes, idx2label ,explainer,tokenizer,only_adds=False):
    ...
    for i in item.manual_features:
        x.append(float(i))
    x = np.array(x)
    exp = explainer.explain_instance(x, predict_fn=pred,
                                num_features=1038, top_labels=1,
                                num_samples=5000)
    try:
